...
function handleConfig(ev) {
    ...
    parseDefaultSettings(luciConfig)
        .then(function(settings) {
            var arglist, keylist = Object.keys(settings),
                res, cstrs = {},
                node = [],
                body;
            ...
            body = [
                E('p', {}, _('Configure the default values for luci-app-wrtbwmon-ng.')),
                E('div', {}, node),
                E('div', { 'class': 'right' }, [
                    E('div', {
...
function renameFile(str, tag) {
    var n = str.lastIndexOf('/'),
        fn = n > -1 ? str.slice(n + 1) : str,
        dir = n > -1 ? str.slice(0, n + 1) : '';
    var n = fn.lastIndexOf('.'),
        bn = n > -1 ? fn.slice(0, n) : fn;
    var n = fn.lastIndexOf('.'),
        en = n > -1 ? fn.slice(n + 1) : '';
...
function updateTable(tb, values, placeholder, settings) {
    var fragment = document.createDocumentFragment(),
        nodeLen = tb.childElementCount - 2;
    var formData = values[0],
        tbTitle = tb.firstElementChild,
        newNode, childTD;
